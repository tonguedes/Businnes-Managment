<div class="py-10">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse ($grupos as $grupo)
                            <tr wire:key="grupo-{{ $grupo->id }}" class="hover:bg-indigo-50/30 transition duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $grupo->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-base text-gray-800">{{ $grupo->nome }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $grupo->created_at->format('d/m/Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                    <button wire:click="openModal({{ $grupo->id }})" class="text-indigo-600 hover:text-indigo-800 font-medium transition duration-150">
                                        Editar
                                    </button>
                                    <button wire:click="confirmDelete({{ $grupo->id }}, '{{ $grupo->nome }}')" class="text-red-600 hover:text-red-800 font-medium transition duration-150">
                                        Excluir
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500 bg-gray-50/50">
                                    ...
                                    <h3 class="mt-2 text-sm font-semibold text-gray-900">Nenhum grupo encontrado</h3>
                                    <p class="mt-1 text-sm text-gray-500">Comece criando um novo grupo econômico.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @include('livewire.partials.modal-form')
